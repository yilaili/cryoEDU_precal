...
'''
Submit relion 3D refinement job.
Inputs: 1. Path of the *particles.star file from particle extraction,
        2. Name of the 3D refinement directory,
        3. Path of the reference volume,
        4. Diameter of the mask,
        5. Whether to do ctf (default is False),
        6. Initial low pass filter for ref (default is -1),
        7. Symmetry (default is C1),
        8. healpix_order (default is 2).
Output: 3D refinement results, saved in the output directory.
'''
...
def setupParserOptions():
    ...
    ap.add_argument('-o','--output', default='Refine3D',
                    help="Name of the directory where the outputs of 3D refinement are stored.")
    ap.add_argument('-p', '--program', default='relion_Refine3D',
                    help='The program to use to do particle extraction. Currently only supports relion_Refine3D.')
    ...
    ap.add_argument('-d', '--diameter',
                    help="Diameter of the particle to be used in 3D refinement (in Angstrom).")
    ap.add_argument('--ctf', action='store_true',
                    help="Whether to do ctf correction. Default is FALSE.")
    ap.add_argument('--ini_high', default='30',
                    help="Initial low pass filter for ref (default is 30).")
    ...
    ap.add_argument('--jobname', default='Refine3D',
...
def editjobconfig(
    ...
    parameters = job_config['relion_Refine3D']['parameters']
...
def parse_config(job_config):
    ...
    '''
    command = job_config['relion_Refine3D']['command']
    parameters = job_config['relion_Refine3D']['parameters'].copy() # make a copy rather than modify in place.
...
def save_pipeline(job_config, save_path):
    ...
    '''
    pipeline = {
        'input': job_config['relion_Refine3D']['parameters']['--i'],
        'output': job_config['relion_Refine3D']['parameters']['--o']
        }
...
def check_good(refine_dir):
    '''
    Currently only supports relion 3D refinement.
    Check if 'run_model.star' file exists.
    '''
    return os.path.isfile(os.path.join(refine_dir, 'run_model.star'))
...
def submit(**args):
    ...
    job_config_file = 'config/config_refine3d.json'
    ...
    # make the subdirectory under the output directory (Refine3D)
    try:
...
def check_output_good(outname, **args):
    ...
    with open(os.path.join(projdir, '%s_%s.log' %(args['program'], outname)), 'a+') as f:
        if isgood:
            f.write('3D refinement for %s has finished.\n'%outname)
        else:
